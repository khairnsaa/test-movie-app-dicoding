...
describe('Liking A Movie', () => {
  ...
  it('should not add a movie again when its already liked', async () => {
    ...
    await FavoriteMovieIdb.putMovie({ id: 1 }); // menambahkan film
    document.querySelector('#likeButton').dispatchEvent(new Event('click')); // simulasikan like button di klik
    // mengecek agar tidak ada film yang ganda
    expect(await FavoriteMovieIdb.getAllMovies()).toEqual([{ id: 1 }]);

    FavoriteMovieIdb.deleteMovie(1);
  });
  it('should not add movie when it has no id', async () => {
